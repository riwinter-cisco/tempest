...
from onep.interfaces import InterfaceCreateDeleteListener
from onep.interfaces import InterfaceCreateDeleteFilter
...
from onep.interfaces import VlanConfig
from onep.element import SessionConfig
from onep.element import SyslogFilter
from onep.element import SyslogListener
from onep.interfaces import Vlan
from onep.element import CLIListener, CLIEvent, CLIFilter
...
class NxMgr(object):
    ...
    Methods of controlling/monitoring the Nexus switch are:
        ...
        - CLI
    """
    def __init__(self, switch_ip, username='admin', password='password', api_version='latest', sw_type='3k'):
    ...
    def connect(self):
        ...
        """
        raise NotImplementedError("NxMgr connect not implemeted")
    ...
    def add_interface_state_listener(self, listener, filter, event_type, client_data):
        """
        :param filter:
        :param event_type:
        :param client_data:
        :return:
        """
        raise NotImplementedError("NxMgr add_interface_state_listener not implemented")
...
class NxOnePMgr(NxMgr):
    ...
    def connect(self):
        ...
        self.session_config.set_tls_pinning(self.tls_pinning_file, PinningHandler(self.tls_pinning_file))

        ## Create the OneP NetworkElement
        self.nx_mgr = NetworkElement(self.switch_ip, "NxOnePMgr-{0}".format(os.getpid()))
        self.session_handle = self.nx_mgr.connect(self.username, self.password, self.session_config)

        ## xml collector to verify configuration
        self.syslog_xml_collector = NxSyslogCollector(self.nx_mgr, "sendCmd2Parser")
        ## traceback collector to verify No Tracebacks occcured during the test
        self.syslog_traceback_collector = NxSyslogCollector(self.nx_mgr, "Traceback")
        ...
        if sd_list is None or len(sd_list) == 0:
            ...
            for sd in sd_list:
                logging.debug("NetworkElement IPAddress = {0} ".format(sd.network_element.host_address))
                services = sd.service_set_list
                if services is not None:
                    for serviceName in services:
                        logging.debug("Service Name: {0}".format(ServiceSetDescription.ServiceSetName.enumval(serviceName)))
                        logging.debug("Versions: {0}".format(services.get(serviceName)))
    ...
    def get_interface_names(self):
        ...
        local_filter = InterfaceFilter(None, interface_types.ONEP_IF_TYPE_ETHERNET)
    ...
    def get_network_interface(self, interface_name):
        ...
        interface_types = NetworkInterface.InterfaceTypes
        if_filter = InterfaceFilter(None, interface_types.ONEP_IF_TYPE_ETHERNET)
    ...
    def get_num_interface_events(self, interface_name, vlan_id):
        if self.nx_mgr is None:
            return None
    ...
    def add_interface_state_listener(self, listener, filter, event_type, client_data):
        self.nx_mgr.add_interface_state_listener(listener, filter, event_type, client_data)

    @property
    def trace_backs(self):
        self.syslog_traceback_collector.num_msgs
        return None
...
class NxInterfaceVlanEventMonitor(InterfaceVlanListener):
    ...
    def __init__(self, nx_mgr, interface_name):
        ...
        interface_types = NetworkInterface.InterfaceTypes
        target_network_interface = self.nx_mgr.get_network_interface(interface_name)
        self.client_data = "NxMgr-{0}".format(interface_name)

        self.if_filter = InterfaceVlanFilter(interface=target_network_interface,
                                             interface_type=interface_types.ONEP_IF_TYPE_VLAN,
        ...
        self.nx_mgr.add_interface_state_listener(self, self.if_filter,
                                                 InterfaceStatus.InterfaceVLANEventType.ONEP_IF_VLAN_EVENT_ANY,
                                                 self.client_data)
    @property
    def num_events(self):
    ...
    def handle_event(self, event, client_data):
        ...
        LOG.debug("    Type         :\t{0}".format(event.interface.interface_type))
...
class NxSyslogCollector(SyslogListener):
    ...
    def __init__(self, nx_mgr, pattern, period=1000, occurs=1):
        ...
        :param occurs: The number of time the pattern is seen before the event is triggered
        """
        ...
        self.syslog_filter.priority = NetworkElement.OnepSyslogSeverity.ONEP_SYSLOG_NOTICE
        self.syslog_filter.occurs = occurs
        self.syslog_event_handle = self.nx_mgr.add_syslog_listener(self, self.syslog_filter, "Syslog-client-{0}".format(pattern))

    def handle_event(self, event, client_data):
        """
        :param event: The SyslogEvnet object received
        :param client_data: The client data configured when registering the listener
        """
        if event.msg_count is 1:
            self.messages.append(event.message)
        else:
            raise NotImplementedError("Handling multiple messages from SyslogEvent {0}".format(client_data))
...
class NxSyslogListener(SyslogListener):
    ...
    def handle_event(self, event, client_data):
        ...
        LOG.debug("  Priority       = {0}".format(NetworkElement.OnepSyslogSeverity.enumval(event.priority)))
        LOG.debug("---------------------------")
...
class NxJsonMgr(NxMgr):
    ...
    def __init__(self, switch_ip, **kwargs):
        ...
        self.headers['Content-Type'] = "application/{0}".format(self.api_format)
...
class NetworkInterfaceNotFoundError(Exception):
    ...
    def __init(self, interface_name, error_msg):
        super(NetworkInterfaceNotFoundError, self).__init__(interface_name, error_msg)
        self.interface_name = interface_name
