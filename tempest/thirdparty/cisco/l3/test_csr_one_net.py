...
class TestCSROneNet(manager.NetworkScenarioTest):
    ...
    def setUp(self):
        ...
        # TODO(riwinter): Put Imix traffic in tempest config
        ...
        #
        # Needed so that the test can control what hypervisor a
        # server is started on.
        #
        self.hypervisors_list = self.hypervisor_client.get_hypervisor_list()
        self.assertGreaterEqual(len(self.hypervisors_list[1]), 2,
                                "Not enough hypervisors to run the test")

        # Verify the hypervisors are operational and make a list
        # of them for later use
        self.hypervisors = []
        for hypervisor in self.hypervisors_list[1]:
            if hypervisor['status'] == 'enabled' and hypervisor['state'] == \
                    'up':
                self.hypervisors.append(hypervisor)

        # Need at least two(2) operational hypervisors
        self.assertGreaterEqual(len(self.hypervisors), 2,
                                "Not enough operational hypervisors to run "
                                "the test")
        ...
        self._add_host(self.aggregates[0]['id'],
                       self.hypervisors[0]['hypervisor_hostname'])

        self._add_host(self.aggregates[1]['id'],
                       self.hypervisors[1]['hypervisor_hostname'])

        # Create each server on a seperate hypervisor
        name = data_utils.rand_name('server')
        server_dict = self._create_server(name, self.network, zone='Zone1')
        self.servers[server_dict['server']['id']] = server_dict['keypair']

        name = data_utils.rand_name('server')
        server_dict = self._create_server(name, self.network, zone='Zone2')
        self.servers[server_dict['server']['id']] = server_dict['keypair']
        ...
        self._check_tenant_network_connectivity_wrapper()
    ...
    def _check_tenant_network_connectivity_wrapper(self):
    ...
    def _check_server_connectivity(self, floating_ip, address_list):
        ...
        private_key = self.servers[
            self.floating_ip_tuple.server['id']]['private_key']
    ...
    def test_csr_one_net(self):
        ...
        for net in current_nets:
            ...
            LOG.debug("  Provider Seg ID: "
                      "{0}".format(net['provider:segmentation_id']))
        ...
        for i in range(0, 3):
            try:
                ping_result = \
                    self.ping_ip_address(floating_ip.floating_ip_address)
                LOG.debug("Ping result : {0}".format(ping_result))
                time.sleep(1)
        ...
        self.assertTrue(ping_result,
                        "Ping of floating IP {0} failed"
                        .format(floating_ip.floating_ip_address))
